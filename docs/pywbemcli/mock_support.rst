...
:ref:`pywbem:Mock WBEM server` which defines a local in server WBEM server environment.
The mock sever provides WBEM server responses to pywbmcli commands. The
configuration and contents of the mock server are defined by the pywbemcli
:ref:`--mock-server general option` and the mock WBEM server configuration
can be saved as a named connection.
...
repository are in the pywbemcli process so its life-cycle is the life-cycle of
a single pywbemcli execution (command line or interactive session) and it must
be recreated or restored from a pywbemcli provided cache (see: TODO) using the :ref:`--name
general option` (if the connection definition was saved).

The mock WBEM server environment can include CIM namespaces, CIM
...
created in pywbemcli rather than a real WBEM server.

The created mock WBEM server has an in-memory repository for the
CIM objects defined by the mock script (qualifier declarations, classes, and
...
sequence of the options on the command line. The  :ref:`--mock-server general option`
is mutually exclusive with the :ref:`--server general option` and
:ref:`--name general option`, since each defines a target WBEM server.
...
.. code-block:: python
    ...
    # Interface from pywbemcli for both old-style and new-style interfaces
    if sys.version_info >= (3, 5):
        # New-style setup
...
file are saved(i.e. where the the general option  ``-C connection-file-name``
is **not** used.)
...
Mock server caches for each named connection are saved in the directory ``.pywbemcli_mockcache``
each in its own subdirectory that includes the connection name.

...
def check_output(cmd_args, situation, msg, verbose):
    ...
    avoiding that the (short-lived) pywbemlistener 'start' command times out
    ...
    try:
        if start_pos is not None:
            ...
            # run command that is launched by the start command. If file handles
            ...
            # command is implemented not via pipes, but via files by

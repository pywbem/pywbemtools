...
was called.  Since all tests are run with the  env var 'PYWBEMCLI_FILES_DIR'
(it existence is validated in clitestextensions.py), any existing user
connections file is not touched.  The location of the test default connections
file and mock cache. is defined by the 'PYWBEMCLI_FILES_DIR' value which must
be a directory. For tests that env var is set in the Makefile.
"""

...
# Click issue #1231:
# The Click package on Windows writes NL at the Python level as '\r\r\n' at the
# level of the Windows shell under some cases, and Popen() with
# universal_newlines=True does not handle this case.
# To circumvent the error, pywbemtools specifies universal_newlines=False and
# performs the universal newline conversions itself (it does what Popen() does,
# and in addition replaces the incorrectly created CRCRNL sequence).
# This error has been fixed in Click 7.1, but Click 7.1 has also removed
# support for Python 3.4 so pywbemtools now requires Click 7.1 where possible
# and still needs the circumvention code when below Click 7.1 and on Windows.
...
CLICK_ISSUE_1231 = CLICK_VERSION.release < (7, 1) and sys.platform == 'win32'
...
def execute_pywbemcli(args, env=None, stdin=None, verbose=None, condition=True):
    ...
    proc = Popen(cmd_args, shell=shell, stdin=stdin_stream,

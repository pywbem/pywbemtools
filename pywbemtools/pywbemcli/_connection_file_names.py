...
# Env var that defines an alternate directory in which
# the default connection file and connection cache directory are defined.
# If PYWBEMCLI_ALT_FILES_HOME_DIR directory for these files is
# user home directory (i.e. '~')
PYWBEMCLI_FILES_DIR_ENVVAR = 'PYWBEMCLI_FILES_DIR'
...
if os.getenv(PYWBEMCLI_FILES_DIR_ENVVAR):
    alt_home_dir_path = os.getenv(PYWBEMCLI_FILES_DIR_ENVVAR)
    if os.path.isdir(alt_home_dir_path):
        DEFAULT_CONNECTIONS_DIR = alt_home_dir_path
    elif os.path.isfile(alt_home_dir_path):
        raise Abort(
            f"Alternate files home path create: {alt_home_dir_path} defined "
            f"by envvar {PYWBEMCLI_FILES_DIR_ENVVAR} failed. File with "
            "same name exists")
    else:
        try:
            ...
            raise Abort(
                ...
                f"defined by envvar {PYWBEMCLI_FILES_DIR_ENVVAR} failed. "
                f"Exception: {oe}")
else:
    DEFAULT_CONNECTIONS_DIR = os.path.expanduser("~")

    assert DEFAULT_CONNECTIONS_DIR is not None  # TODO remove

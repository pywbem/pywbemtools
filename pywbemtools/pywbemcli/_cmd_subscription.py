...
adding, displaying, and remvoes destinations, filters, and subscriptions from
a WBEM server.
...
def subscription_add_destination(context, identity, **options):
    ...
    define the Name key property of the the created instance.  If the
    instance is to be owned by the current SubscriptionManager, pywbemcli
    creates a Name property value with the format:
    ...
    becomes the value of the Name property.

    Owned destinations are added or updated conditionally: If the
    destination instance to be added is already registered with
    ...
    If a listener destination instance with the specified or generated 'Name'
    property already exists, the method returns an exception. This is a more
    strict behavior than what a WBEM server would do, because the 'Name'
    property is only one of four key properties.
    """
...
def subscription_add_subscription(context, destination_identity,
    ...
    based on the DESTINATION and FILTER arguments which define the Name key of
    the 'Handler' and 'Filter' association instance names of the subscription
    ...
    is the full value of the Name property for permanent destinations and is a
    component of the Name property for owned instances. If just the identity is
    used, this will result in multiple destinations being found if the same
    string is defined as the identity of an owned and permanent destination.

    The required FILTER argument defines the existing filter instance
    that will be attached to the 'Filter' reference of the association class.
    This argument may consist of either the value of the Name property of the
    ...
    component of the Name property for owned instances. If just the identity is
    used, this will result in multiple filters being found if the same string
    ...
    indication filter and listener destination may be owned or permanent
...
def subscription_list_destinations(context, **options):
    ...
    The data display is determined by the --detail, --paths, and --summary
...
def subscription_remove_destination(context, identity, **options):
    ...
    The required IDENTITY argument is the value of the IDENTITY argument used
    to create the destination. This is the value of the Name property for
    permanent destinations and a component of the Name property for owned
    destinations.
...
def subscription_remove_filter(context, identity, **options):
    ...
    used to create the filter or may be the full name of the filter Name
...
def subscription_remove_subscription(context, destination_identity,
    ...
    This may result in more than one subscription which then will be removed.
    """
...
class CmdSubscriptionManager(object):
    ...
    def add_destination(self, listener_url, owned, destination_id,
        ...
        # Invalid input parameters exception
            ...
            if er.status_code == CIM_ERR_ALREADY_EXISTS:
                raise click.ClickException(
                    "add-destination failed. Destination listener_{0} "
    ...
    def get_filters_for_type(self, type_choice):
        """
        Get list of destinations based on choice where choice is all, permanent,
        or owned.
        """
    ...
    def get_subscriptions_for_type(self, type_choice):
        """
...
def cmd_subscription_add_destination(context, identity, options):
    ...
    if owned_opt and not destination_result['Name'].endswith(destination_id):
        click.echo(
            ...
            format(get_owned_str(owned_opt), destination_id,
...
def cmd_subscription_list(context, options):
    ...
    all_subscriptions = csm.get_subscriptions_for_type('all')
    all_destinations = csm.get_destinations_for_type('all')
    all_filters = csm.get_filters_for_type('all')
...
def cmd_subscription_list_destinations(context, options):
    ...
    destinations = csm.get_destinations_for_type(ownedtype_opt)
    ...
    if output_format_is_table(output_format):
        headers = ['Ownership', 'Identity', 'Name', 'Destination',
        ...
        title = "Indication Destinations: submgr-id={0}, svr-id={1}, " \
            "type={2}". \
            format(csm.submgr_id, csm.server_id, ownedtype_opt)
...
def cmd_subscription_list_filters(context, options):
    ...
    filter_type_opt = options['type']
    details_opt = options['detail']

    filters = csm.get_filters_for_type(filter_type_opt)
    ...
    if filters:
        if output_format_is_cimobject(output_format):
            ...
            headers = ['Ownership', 'identity', 'Name', 'Query',
                       'Query\nLanguage', 'Source\nNamespaces',
                       'Subscription\nCount']
            if options['detail']:
                headers.extend(
            ...
            title = "Indication Filters: submgr-id={0}, svr-id={1} type={2}". \
                format(csm.submgr_id, csm.server_id, filter_type_opt)
        ...
        if context.verbose:
            ...
            click.echo("No matching filters for server_id={0} "
                       "type {1}".format(csm.server_id, filter_type_opt))
...
def cmd_subscription_list_subscriptions(context, options):
    ...
    svr_subscriptions = csm.get_subscriptions_for_type(options['type'])
...
def verify_instances_removal(paths, instance_type):
